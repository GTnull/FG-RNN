...
from model.fgrnn_model import FGRNNModel


class FGRNNSupervisor(object):
    def __init__(self, adj_mx, **kwargs):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('FGRNN', reuse=False):
                self._train_model = FGRNNModel(is_training=True, scaler=scaler,
                                               batch_size=self._data_kwargs['batch_size'],
                                               adj_mx=adj_mx, **self._model_kwargs)

        with tf.name_scope('Test'):
            with tf.variable_scope('FGRNN', reuse=True):
                self._test_model = FGRNNModel(is_training=False, scaler=scaler,
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            if filter_type == 'random_walk':
                ...
                filter_type_abbr = 'DR'
            run_id = 'fgrnn_%s_%d_h_%d_%s_lr_%g_bs_%d_%s/' % (
